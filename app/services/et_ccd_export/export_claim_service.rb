...
module EtCcdExport
  class ExportClaimService
    ...
    def do_export(export)
      config = export.dig('system', 'config')
    ...
    def exchange_service_token_for(export)
      url = export.dig('system', 'config', 'idam_service_token_exchange_url')
      resp = RestClient.post(url, {microservice: "et"}.to_json, content_type: 'application/json')
      resp.body
    end

    def exchange_user_token_for(export) config = export.dig('system', 'config')
